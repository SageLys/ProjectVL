...
describe('equipmentSystem · 方案A独立装备', () => {
  ...
  it('装备卡可拖到战场消耗释放并清空装备格', () => { const s=freshState(); s.equipment[1]=card('range',5); expect(consumeCard(s,config,rng,1,120,240,'equipment')).toContainEqual({type:'skillConsumed',cardType:'range',star:5,x:120,y:240}); expect(s.equipment[1]).toBeNull(); expect(s.consumes).toBe(1); });
  it('消耗空装备格无副作用', () => { const s=freshState(); expect(consumeCard(s,config,rng,0,120,240,'equipment')).toEqual([]); expect(s.consumes).toBe(0); });
});
