...
export const ATOM_CONTRACT = {
  ...
  aoeOnHit: {
    ...
    allowedTriggers: ['onFire', 'onHit', 'onKill', 'onBreach', 'interval'],
    supports: { equip: true, consume: true },
    emitsEvents: true,
  },
  beamMorph: {
  ...
  aura: {
    ...
    params: {
      ...
      follow: { type: 'enum', enum: ['densestCluster', 'nearestEnemy'] },
      followLerp: { type: 'number', default: 0.5, min: 0, max: 1 },
      radiusOverTime: { type: 'record', note: 'duration 内从 from 到 to 线性插值' },
  ...
  groundZone: {
    ...
    params: {
      ...
      innerRadius: { type: 'number', min: 0, note: 'shape=ring 的内径；未声明 = radius × 0.5' },
      radiusOverTime: { type: 'record', note: 'duration 内从 from 到 to 线性插值' },
      lineFrom: { type: 'enum', enum: ['turretToPoint', 'bulletPath'] },
      color: { type: 'string', note: '未声明 = 继承来源卡的主题色' },
