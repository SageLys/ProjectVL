...
import type { EvolutionRecipesConfig, GodDef } from '../config/types';
...
function renderIssues(container: HTMLElement, options: ContextPanelOptions): void {
  ...
  const fallback: EditorDomain = options.selection.kind === 'card' ? 'skills' : 'gods';
...
function renderReverseReferences(container: HTMLElement, options: ContextPanelOptions): void {
  ...
  if (options.selection.kind === 'card') {
    ...
    if (god) lines.push(`锚点卡 ${god.anchorCardIds.length} 张：${god.anchorCardIds.join('、')}`, `可变卡 ${god.variableCardIds.length} 张：${god.variableCardIds.join('、')}`);
  }
...
function renderTextSlots(container: HTMLElement, options: ContextPanelOptions): void {
  ...
  if (options.selection.kind === 'card') {
    ...
    const root = record(options.texts.gods)[options.selection.id];
    flattenStrings(root, `$.texts.gods.${options.selection.id}`, slots);
