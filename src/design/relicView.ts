...
import { editableMechanismBlock, renderRelicForm, type MechanismEditingOptions } from './mechanismEditor';
import type { BuildScalingAxis } from '../config/types';

export function renderRelicView(
  ...
  rarityOptions: RelicDef['rarity'][] = [relic.rarity],
  axisOptions: BuildScalingAxis[] = relic.effects.map(effect => effect.axis),
): void {
  ...
  const mechanismRead = el('div', 'relic-mechanism-read');
  mechanismRead.append(badges, el('h2', '', '构筑效果'), table, el('p', '', `最大叠加：${view.maxStacks} 层`));
  ...
  mechanismRead.append(pool);
  const relicPath = article.dataset.configPath;
  article.append(editableMechanismBlock(mechanismRead, relicPath, mechanism, editor => {
    if (mechanism) renderRelicForm(editor, relic, relicPath, rarityOptions, axisOptions, mechanism);
  }, '', '编辑遗物机制'));
