...
const ATOM_PARAM_LABELS: Record<string, string> = {
  ...
  width: '宽度',
  preferRelay: '优先中继',
  relayDamageBonus: '中继增伤',
  relayBonusCap: '中继增伤上限',
  maxCount: '场上上限',
  chainRelay: '链路中继',
  onDeathEffects: '死亡效果',
  intervalSeconds: '周期秒数',
  intervalEffects: '周期效果',
  auraRadius: '自带光环半径',
  auraEffects: '自带光环效果',
  secure: '安全掉落',
  by: '计量来源',
  ...
  'mergeMaterialRefund.count': '返还张数',
  'mergeMaterialRefund.scope': '返还来源',
  'mergeMaterialRefund.star': '返还素材星级',
  'wildcardRewardBonus.count': '额外万能卡张数',
  'wildcardRewardBonus.scope': '奖励来源',
...
const DOMAIN_FIELD_LABELS: Record<EditorDomain, Record<string, string>> = {
  combat: {
    ...
    'weaponFusion.impactShare': '融合爆炸预算占比',
...
const COMMON_FIELD_LABELS: Record<string, string> = {
  ...
  triggerParams: '触发参数', seconds: '间隔秒数', requiresSource: '来源限定', requiresStatus: '状态限定',
  cooldownSeconds: '冷却秒数', type: '类型', typePolicy: '类型策略', types: '类型表',
...
const ENUM_LABELS: Record<string, Record<string, string>> = {
  ...
  tag: { projectile: '弹道', control: '控制', domain: '领域', defense: '防御', utility: '功能' },
  phase: { intermission: '波间' },
